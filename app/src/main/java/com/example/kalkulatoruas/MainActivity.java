...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        radioOperator = findViewById(R.id.operasi);
        sh = this.getSharedPreferences("history", Context.MODE_PRIVATE);
        myrey = findViewById(R.id.reyRiwayat);
        mylist = new ArrayList<>();
        temp = sh.getAll().size()+1;
        myAdapter = new AdapterRiwayat(mylist,this,sh);
    ...
    public void getArray(String no,String rwyt){
        try{
            myrey.setAdapter(new AdapterRiwayat(mylist, this, sh));
            myrey.setLayoutManager(new LinearLayoutManager(this));
            mylist.add(new Riwayat(no,String.valueOf(rwyt)));
        }catch (Exception e){
            e.printStackTrace();
